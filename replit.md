...
Runs fully independent of Replit. Files: `Dockerfile`, `docker-compose.yml`, `.env.example`, `.dockerignore`.
...
- **DB schema — app-managed startup migrations:** on every boot the API server runs the idempotent DDL in `artifacts/api-server/src/lib/migrate.ts` (CREATE TABLE/INDEX IF NOT EXISTS + ALTER TABLE ADD COLUMN IF NOT EXISTS) before listening, and exits if migration fails. A fresh database gets the full schema (including the `session` table — `createTableIfMissing:false`) and an existing database is upgraded automatically, so `docker compose up -d --build` is a complete deploy with **no manual SQL ever**. When adding a schema change: update `lib/db/src/schema/*.ts` AND add the matching statement to `migrate.ts` (new columns go in both the baseline CREATE TABLE and an incremental ALTER). The old `docker/postgres/initdb/` init-SQL mechanism was removed — migrate.ts is the single source of truth.
...
- The `session` table is created by the startup migrations in `migrate.ts` (needed because `createTableIfMissing:false`); no manual setup on new DBs.
