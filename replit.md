...
- `artifacts/api-server/src/assets/vnc-handler-install.bat` — one-time installer (run as Administrator) that registers the `vnc://` scheme for viewers that don't do it themselves (TightVNC/UltraVNC/TigerVNC; RealVNC registers it natively). Downloads `vnc-launch.ps1` to `%ProgramData%\OBTV Fleet\` and points `HKCR\vnc\shell\open\command` at it via `powershell -File "...vnc-launch.ps1" "%1"`. Dashboard URL is templated at download time from the request origin. Served at `<dashboard>/api/agent/vnc-handler.bat`.
