...
**Jump Desktop** is separate: its scheme is `jump://?host=...&protocol=...`. With no `protocol` it defaults to **RDP**. This shop's machines are managed by **Jump Desktop Connect, which connects over Fluid**, so use `protocol=fluid`. **CRUCIAL: Connect/Fluid computers are matched by their registered NAME (the machine hostname), NOT by IP** — passing an IP makes Jump Desktop open but never connect ("opens the app, not the client"). Use `jump://?host={hostname}&protocol=fluid` (no port — Fluid resolves the named computer via the account). `protocol=vnc` is rejected ("VNC is not supported — use Fluid"). Do NOT assume Jump Desktop == VNC: it is its own protocol stack. Mnemonic for this shop: **Jump Desktop = hostname; VNC = IP**.
